...
namespace HAL
{
    ...
    public:
        typedef enum
        {
            PORTA = (uint16_t)GPIOA_BaseAddress,
            PORTB = (uint16_t)GPIOB_BaseAddress,
            PORTC = (uint16_t)GPIOC_BaseAddress,
            PORTD = (uint16_t)GPIOD_BaseAddress,
            PORTE = (uint16_t)GPIOE_BaseAddress
        } Port_t;
        ...
        static void SetInterrupt(Port_t Port, IntEdge_t Edge)
        {
            ...
            case PORTC:
                ...
                break;
            case PORTD:
             // if(EXTI->CR1 & EXTI_CR1_PDIS != EXTI_CR1_PDIS)
              //{
