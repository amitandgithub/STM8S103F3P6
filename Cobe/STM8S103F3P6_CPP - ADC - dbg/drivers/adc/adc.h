...
public:       
  ...
  {    
    ...
    Disable_Schmit_Trig(ADC_CH_5);
    Disable_Schmit_Trig(ADC_CH_6);
    
    
    ADC1->CSR &= (uint8_t)(~ADC1_CSR_CH);
    ADC1->CSR |= (uint8_t)(ADC_CH_6);  
    
    SetAlignment(ADC_Align_RIGHT);
    Enable_ScanMode();
    //SetMode(ADC_Mode_Continous);
   ...
   // CLK->PCKENR2 |= (uint8_t) CLK_PCKENR2_ADC; 
    ...
    //SetMode(ADC_Mode_Continous);
    //SetPrescalar(ADC_PSC_2);
    
    //Disable_Ext_Trig(Ext_Trig_GPIO);
    PowerOn();
  ...
  void StartConversion(ADC_Ch_t ch)
  {
    ADC1->CSR &= (uint8_t)(~ADC1_CSR_CH);
    ADC1->CSR |= (uint8_t)(ch);    
    PowerOn();
  ...
  static void SetAlignment(ADC_Align_t Align) { ADC1->CR2 |= (uint8_t)(Align); }
