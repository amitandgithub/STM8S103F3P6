...
{ 
  ...
  while(1)
  {
    ...
    if(Ir_Decoder.GetState())
    {
      //Ir_Decoder.Decode(&Results);
      Results = Ir_Decoder.Decode();
...
void IR_Extender(void)
{
  ...
  while(1)
  {
   ...
   Ir_Decoder.Run();
    ...
    State = Ir_Decoder.GetState();
    
    if( (State == IR_Decoder_t::IDLE) || (State == IR_Decoder_t::REPEATED_CODE) )
    {
      Results = Ir_Decoder.Read();
      
      if(Results)
      {
        WT_to_Samsung(Results);
      }
      //Ir_Decoder.Start();     
    }
  }  
...
void WT_to_DDC(uint32_t code)
{
  ...
  else if(code == WT_VOL_UP)
  {
    Ir_Encoder.TxProtocol(DDC_VOL_UP);
  }
  else if(code == WT_VOL_DOWN)
  {
