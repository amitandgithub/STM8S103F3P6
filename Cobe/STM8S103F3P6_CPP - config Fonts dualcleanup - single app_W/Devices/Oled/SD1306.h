...
namespace Devices
{
  typedef enum
  {
    Font_11x18 = ((11+1) * (18+1)) / 8,
    Font_16x26 = ((16+1) * (26+1)) / 8,
  }font_t;
  
    template <typename I2c, uint8_t Device_Address, bool eeprom_font_buffer = false > // Font_11x18 -> 3420 bytes in flash

    class SSD1306  // Saved around 75b flash, 1b RAM for making static
    {
    public:
        static const uint8_t BufSize = Font_16x26; 
        ...
        static uint16_t Font_Current_address;
        ...
        static void SetFont(font_t font)
        {  
           if(font == Font_11x18) 
            {
              m_FontWidth = 11;
        ...
        static void UpdateScreen(uint8_t x, uint8_t y)
        {
          ...
          Cmd( x + m_FontWidth); // Column end address (127 = reset)
          ...
          I2c::Tx(Device_Address, ScreenBuffer, m_BufSize);
        }
        ...
        {          
          ...
          if (color == SSD1306_COLOR_WHITE) {
            ScreenBuffer[x + (y / 8) * (m_FontWidth +1) + 1] |= 1 << (y % 8);
          } else {
            ScreenBuffer[x + (y / 8) * (m_FontWidth +1) + 1] &= ~(1 << (y % 8));
          }
        }
        ...
        {            
            ...
            {
                b = FonData[(ch - 32) * m_FontHeight + i];
                //b = FonData[(ch - 32 -66) * m_FontHeight + i]; // use this for updating big font address2 in eeprom
                for (j = 0; j < m_FontWidth; j++)
                {
                    if ((b << j) & 0x8000)
                    {
        ...
        static void Putc_from_eeprom(char ch, uint8_t x, uint8_t y )
        {
            ...
            AT24C128_t::Read(Font_Current_address + (ch - 32)*m_BufSize, ScreenBuffer,m_BufSize);
        ...
        {        
          char ch = ' ';
           ...
           // Update_Eeprom_Big_Font('b', '~',Font_16x26_start_address2 ); 
            Puts("01234567",7,0,0);            
            Puts("ABCDEFGH",7,0,21);
            Puts("abcdefgh",7,0,42);
            
            SetFont(Font_16x26);//Font_16x26
            Puts("01234567",7,0,0);            
    ...
    template <typename I2c, uint8_t Device_Address, bool eeprom_font_buffer >
      uint16_t Devices::SSD1306<I2c, Device_Address, eeprom_font_buffer >::Font_Current_address;
    
    template <typename I2c, uint8_t Device_Address, bool eeprom_font_buffer >
      uint8_t Devices::SSD1306<I2c, Device_Address, eeprom_font_buffer >::m_FontWidth;
    
    template <typename I2c, uint8_t Device_Address, bool eeprom_font_buffer >
      uint8_t Devices::SSD1306<I2c, Device_Address, eeprom_font_buffer >::m_FontHeight;    
    
    template <typename I2c, uint8_t Device_Address, bool eeprom_font_buffer >
      uint8_t Devices::SSD1306<I2c, Device_Address, eeprom_font_buffer >::m_BufSize;
    
    template <typename I2c, uint8_t Device_Address, bool eeprom_font_buffer >
      uint8_t Devices::SSD1306<I2c, Device_Address, eeprom_font_buffer >::ScreenBuffer[Devices::SSD1306<I2c, Device_Address, eeprom_font_buffer >::BufSize+1];
}
