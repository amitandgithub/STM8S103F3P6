    ...
    class PowerMonitor
    {
      ...
      static void UpdateMode(void)
      {
        ...
        else if(Mode == 1)
        {
          ...
          ssd1306_t::Puts("I ",2,0,LINE_2_16_26);
        }
      }
      ...
      {        
        // Run every 125 ms
          ...
          if(Mode == 0)
          {
            memset((uint8_t*)&DispBuf[6],' ',COLS_11_18-6);
            Utils::ftoa(Power.Voltage,&DispBuf[6],2,' ');
            ssd1306_t::Puts(&DispBuf[6],COLS_11_18-6 ,6*11,LINE_1);
            
            memset((uint8_t*)DispBuf,' ',COLS_11_18);
            Utils::ftoa(Power.Current,DispBuf,2,' ');                 
            ssd1306_t::Puts(DispBuf,COLS_11_18-2,2*11,LINE_2);
            
            memset((uint8_t*)DispBuf,' ',COLS_11_18);
            Utils::ftoa(Power.mAH,DispBuf,2,' ');                 
            ssd1306_t::Puts(DispBuf,COLS_11_18-2,2*11,LINE_3);
          }
          else if(Mode == 1)
          {
            memset((uint8_t*)&DispBuf[0],' ',COLS_16_26);
            Utils::ftoa(Power.Voltage,&DispBuf[0],2,' ');
            ssd1306_t::Puts(&DispBuf[0],COLS_16_26-2 ,2*16,LINE_1);
            
            memset((uint8_t*)DispBuf,' ',COLS_16_26);
            Utils::ftoa(Power.Current,DispBuf,2,' ');                 
            ssd1306_t::Puts(DispBuf,COLS_16_26-2,2*16,LINE_2_16_26);
          }
          else
          {
