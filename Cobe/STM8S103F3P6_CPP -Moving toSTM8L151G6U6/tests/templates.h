...
example<int*,&test> int_example(&test);
example<int&,test> int_ref_example(test);
...
void template_test(void)
{
  ...
  while(1)
  {
    apply(fi,10);
    Call(fi);
    Call(Int_Fx.Static_fi);
    Call(Function<char>::Static_fi);
    //int_example.Set(&test);
    int_example.Set(int_arr);
    //flag::write(1);
