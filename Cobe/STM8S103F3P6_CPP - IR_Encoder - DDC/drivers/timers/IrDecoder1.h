...
class IrDecoder1 : public pulseIn1<Channel,15,0xffff,TimerPol_Rising,34,0> 
{
  ...
  static const uint16_t ONE_SPACE_TOLERANCE = 300;
  static const uint8_t HDR_MARK_POS = 0;
  static const uint8_t HDR_SPACE_POS = 1;
...
//  }
  ...
  {    
    if( PulseIn::SampleIndex >= 2)
    {
      if(((PulseIn::Samples[HDR_SPACE_POS] > RPT_SPACE - ONE_SPACE_TOLERANCE) && (PulseIn::Samples[HDR_SPACE_POS] < RPT_SPACE + ONE_SPACE_TOLERANCE)) )
      {
        PulseIn::Stop();
        //PulseIn::SampleIndex = 0;
        PulseIn::m_Status = 2;
      ...
      {
        delay_ms(60);
        if( !((PulseIn::Samples[HDR_SPACE_POS] > HDR_SPACE - ONE_SPACE_TOLERANCE) && (PulseIn::Samples[HDR_SPACE_POS] < HDR_SPACE + ONE_SPACE_TOLERANCE)) ) 
        {            
          PulseIn::SampleIndex = 0;
        }
      }             
  ...
  */
  ...
  uint32_t Decode(void)
  {
    ...
    uint8_t i;
     ...
     if( (PulseIn::Samples[HDR_SPACE_POS] > HDR_SPACE - ONE_MARK_TOLERANCE) && (PulseIn::Samples[HDR_SPACE_POS] < HDR_SPACE + ONE_MARK_TOLERANCE) ) 
      {          
        // Decode Bits
